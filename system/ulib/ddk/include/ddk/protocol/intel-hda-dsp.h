...
static inline void ihda_dsp_get_dev_info(ihda_dsp_protocol_t* ihda_dsp,
                                         zx_pcie_device_info_t* out_info) {
    ihda_dsp->ops->get_dev_info(ihda_dsp->ctx, out_info);
}

static inline zx_status_t ihda_dsp_get_mmio(ihda_dsp_protocol_t* ihda_dsp,
                                            zx_handle_t* out_vmo,
                                            size_t* out_size) {
    return ihda_dsp->ops->get_mmio(ihda_dsp->ctx, out_vmo, out_size);
}

static inline zx_status_t ihda_dsp_get_bti(ihda_dsp_protocol_t* ihda_dsp, zx_handle_t* out_handle) {
    return ihda_dsp->ops->get_bti(ihda_dsp->ctx, out_handle);
}

static inline void ihda_dsp_enable(ihda_dsp_protocol_t* ihda_dsp) {
    return ihda_dsp->ops->enable(ihda_dsp->ctx);
}

static inline void ihda_dsp_disable(ihda_dsp_protocol_t* ihda_dsp) {
    return ihda_dsp->ops->disable(ihda_dsp->ctx);
}

static inline zx_status_t ihda_dsp_irq_enable(ihda_dsp_protocol_t* ihda_dsp,
                                              ihda_dsp_irq_callback_t* callback,
                                              void* cookie) {
    return ihda_dsp->ops->irq_enable(ihda_dsp->ctx, callback, cookie);
}

static inline void ihda_dsp_irq_disable(ihda_dsp_protocol_t* ihda_dsp) {
