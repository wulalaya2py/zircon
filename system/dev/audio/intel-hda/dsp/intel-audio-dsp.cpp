...
IntelAudioDsp::IntelAudioDsp()
    : ipc_(*this) {
    // Set to MODULE_ID_INVALID
    ::memset(module_ids_, 0xFF, sizeof(module_ids_));
...
int IntelAudioDsp::InitThread() {
    ...
    auto cleanup = fbl::MakeAutoCall([&st, this]() {
        if (st != ZX_OK) {
            DeviceShutdown();
        }
    });
...
void IntelAudioDsp::PausePipelines() {
    ipc_.SetPipelineState(0, PipelineState::PAUSED, true);
    ipc_.SetPipelineState(1, PipelineState::PAUSED, true);
    // Reset DSP DMA
    ipc_.SetPipelineState(0, PipelineState::RESET, true);
    ipc_.SetPipelineState(1, PipelineState::RESET, true);
}
