...
static bool pasm_error_test() {
    BEGIN_TEST;
    auto pasm = PasmTest::CreateTestPasm();
    ASSERT_NONNULL(pasm, "");

    // Try to reserve more than the max number of entries that fit in a page
    for (size_t i = 0; i < Pasm::kEarlyRegionMaxCnt + 2; i++) {
        zx_status_t st = pasm->ReserveAddressSpaceEarly(Pasm::kIoAllocator, i, 1);
        if (i < Pasm::kEarlyRegionMaxCnt) {
            EXPECT_EQ(ZX_OK, st, "");
        } else {
    ...
    EXPECT_EQ(ZX_ERR_BAD_STATE,
              pasm->ReserveAddressSpace(Pasm::kMmioAllocator, 0, 1024u, uptr1), "");
    EXPECT_EQ(ZX_OK, pasm->Initialize(0, PAGE_SIZE, 0, UINT64_MAX, 0, UINT64_MAX), "");

    uintptr_t base = PAGE_SIZE;
    size_t size = PAGE_SIZE;
    EXPECT_EQ(ZX_ERR_NOT_FOUND, pasm->ReserveAddressSpace(Pasm::kMmioAllocator,
                                                          base, size, uptr2),
              "");
    END_TEST;
}
...
static bool pasm_early_alloc_test() {
    ...
    ASSERT_EQ(ZX_OK, pasm->ReserveAddressSpaceEarly(
                         Pasm::kMmioAllocator, test_base, test_size),
              "");
...
static bool pasm_valid_alloc_test() {
    ...
    EXPECT_EQ(ZX_OK, pasm->ReserveAddressSpace(Pasm::kMmioAllocator,
                                               base, size, uptr1),
              "");
    ...
    EXPECT_EQ(ZX_ERR_NOT_FOUND, pasm->ReserveAddressSpace(Pasm::kMmioAllocator,
                                                          base, size, uptr2),
              "");
    EXPECT_EQ(ZX_OK, pasm->ReserveAddressSpace(Pasm::kIoAllocator,
                                               base, size, uptr3),
              "");
    EXPECT_EQ(ZX_ERR_NOT_FOUND, pasm->ReserveAddressSpace(Pasm::kIoAllocator,
                                                          base, size, uptr4),
              "");
    EXPECT_EQ(ZX_OK, pasm->ReserveAddressSpace(Pasm::kIrqAllocator,
                                               base, size, uptr5),
              "");
    END_TEST;
}
...
static bool pasm_cleanup_test() {
    ...
    // Reserve some space, confirm that it was removed
    {
        ...
        EXPECT_EQ(ZX_OK, pasm->ReserveAddressSpace(
                             Pasm::kIrqAllocator, base, size, uptr1),
                  "");
        EXPECT_EQ(ZX_ERR_NOT_FOUND, pasm->ReserveAddressSpace(
                                        Pasm::kIrqAllocator, base, size, uptr2),
                  "");
    }
