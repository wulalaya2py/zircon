...
constexpr const char* region_id_to_str(int region_id) {
    switch (region_id) {
    case Pasm::kMmioAllocator:
        return "mmio";
    case Pasm::kIoAllocator:
        return "io";
    case Pasm::kIrqAllocator:
        return "irq";
    default:
        return "{unknown}";
    }
...
fbl::RefPtr<Pasm> Pasm::instance_;
// Backing memory for the kernel object allocated by Pasm::Create()
static uint8_t instance_buf[sizeof(Pasm)];

// The Pasm class is designed to exist as a singleton for use by the kernel in
...
// PasmTest). Since the Pasm needs to track reserved memory arenas before the
...
zx_status_t Pasm::Create() {
    if (instance_ != nullptr) {
        return ZX_ERR_ALREADY_EXISTS;
    }

    instance_ = fbl::AdoptRef(new (instance_buf) Pasm());
    LTRACEF("%s kernel singleton created.\n", kLogTag);
    return ZX_OK;
}

zx_status_t Pasm::ReserveAddressSpaceEarly(uint32_t region_id, uintptr_t base, size_t size) {
...
zx_status_t Pasm::AddAddressSpace(uint32_t region_id, uintptr_t base, size_t size) {
    AutoSpinLock lock(&lock_);
    return AddAddressSpaceLocked(region_id, base, size);
}

zx_status_t Pasm::AddAddressSpaceLocked(uint32_t region_id, uintptr_t base, size_t size) {
    if (region_id > Pasm::kAllocatorCount) {
...
zx_status_t Pasm::Initialize(uintptr_t mmio_base, size_t mmio_size,
    ...
    auto backing_pool = RegionAllocator::RegionPool::Create(Pasm::kMaxRegionPoolSize);
    if (backing_pool == nullptr) {
        return ZX_ERR_NO_MEMORY;
    }

    for (int region_id = 0; region_id < Pasm::kAllocatorCount; region_id++) {
    ...
    st = AddAddressSpaceLocked(Pasm::kMmioAllocator, mmio_base, mmio_size);
    if (st != ZX_OK) {
        printf("pasm: couldn't add {%" PRIxPTR ", %#lx} to MMIO region: %d\n",
               mmio_base, mmio_size, st);
    }

    st = AddAddressSpaceLocked(Pasm::kIoAllocator, io_base, io_size);
    if (st != ZX_OK) {
        printf("pasm: couldn't add {%" PRIxPTR ", %#lx} to IO region: %d\n", io_base, io_size, st);
    }

    st = AddAddressSpaceLocked(Pasm::kIrqAllocator, irq_base, irq_size);
    if (st != ZX_OK) {
        printf("pasm: couldn't add {%" PRIxPTR ", %#lx} to IRQ region: %d\n", irq_base, irq_size, st);
    }
    ...
    if (early_region_cnt_ > 0) {
        ...
        // To handle the lifecycle of the overall Pasm object we need to hold references
        ...
        early_region_uptrs_ = new (&ac) RegionAllocator::Region::UPtr[early_region_cnt_]();
...
zx_status_t Pasm::ReserveAddressSpace(uint32_t region_id, uintptr_t base, size_t size,
                                      RegionUPtr& out_region) {
    AutoSpinLock lock(&lock_);
    return ReserveAddressSpaceLocked(region_id, base, size, out_region);
}

zx_status_t Pasm::ReserveAddressSpaceLocked(uint32_t region_id, uintptr_t base, size_t size,
                                            RegionUPtr& out_region) {
    if (region_id >= Pasm::kAllocatorCount) {
        return ZX_ERR_INVALID_ARGS;
    }
...
void Pasm::Dump(uint32_t print_id) {
    AutoSpinLock lock(&lock_);
    for (size_t i = 0; i < early_region_cnt_; i++) {
        if (likely(print_id == Pasm::kAllocatorCount) || early_regions_[i].id == print_id) {
    ...
    for (region_id = 0; region_id < Pasm::kAllocatorCount; region_id++) {
        if (likely(print_id == Pasm::kAllocatorCount) || region_id == print_id) {
...
void Pasm::Dump(const char* region_str) {
...
void Pasm::Dump() {
    Dump(Pasm::kAllocatorCount);
}

fbl::RefPtr<Pasm> PasmTest::CreateTestPasm() {
    fbl::AllocChecker ac;
    auto rptr = fbl::AdoptRef(new (&ac) Pasm());
    if (ac.check()) {
        return rptr;
    }
...
static int cmd_pasm(int argc, const cmd_args* argv, uint32_t flags) {
    ...
    if (!Pasm::Get()) {
        printf("No Pasm instance exists!\n");
        return -1;
    }

    // No args passed
    if (argc == 1) {
        Pasm::Get()->Dump(Pasm::kAllocatorCount);
    } else {
        // walk through all tags provided
        for (int i = 1; i < argc; i++) {
            Pasm::Get()->Dump(argv[i].str);
        }
    }
